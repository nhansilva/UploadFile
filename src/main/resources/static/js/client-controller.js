(function(){
        ...
        $scope.uploadProductExcel = function() {
            ...
            if(hasProductExcel) {
                uploadService.uploadProductExcel($scope.token, function(result) {
                    if(result.success) {
                        toastr.info('Your Excel has been uploaded.' + result.info, 'Excel Uploaded');
                        $timeout(function(){
                            loadProducts();
                        }, 1000);
                    } else {
    ...
    app.controller("clientController", ['$timeout', '$log', '$scope', '$route', 'socketService', 'uploadService', 'agentService', 'messageService', controller]);
